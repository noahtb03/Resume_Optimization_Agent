...
def parse_resumes(paths: list[str], client: LLMClient, max_files: int = 3) -> ParseDraft:
    ...
    raw = client.complete(system=PARSE_SYSTEM, user=user, max_tokens=8000)

    try:
        data = extract_json(raw)
    except Exception as e:
        # most common cause: the model's JSON was truncated at the token limit
        hint = ""
        if raw.rstrip().endswith(("," , '"')) or not raw.rstrip().endswith("}"):
            hint = (" The response looks truncated -- the resume may be very long. "
                    "Try parsing fewer resumes at once, or a shorter file.")
        raise ValueError(f"parser model returned unparseable JSON: {e}.{hint}") from e
