...
def select(
    ...
    if skills_spec and skills_spec.enabled:
        # concept map: JD terms that imply whole skill CATEGORIES, so e.g. a
        # "full-stack" or "web" posting surfaces Frontend/Backend skills even when
        # the skill name (React, Flask) doesn't literally appear in the JD.
        category_concepts = {
            "Frontend": {"frontend", "front-end", "fullstack", "full-stack", "web",
                         "ui", "react", "responsive", "client-side"},
            "Backend": {"backend", "back-end", "fullstack", "full-stack", "api",
                        "apis", "server", "rest", "database", "microservices"},
            "AI/ML": {"ml", "ai", "machine", "learning", "model", "models", "llm",
                      "nlp", "genai"},
            "Data": {"data", "analytics", "pipeline", "pipelines", "etl"},
            "Languages": set(),
            "Engineering": {"ci/cd", "testing", "devops", "agile", "git"},
            "Design": {"design", "ui/ux", "ux", "figma", "prototype"},
        }

        def skill_score(s) -> int:
            score = len(_skill_terms(s) & jd_terms)         # direct name/alias match
            concepts = category_concepts.get(s.category or "", set())
            if concepts & jd_terms:                          # category-concept match
                score += 2
            return score

        ranked = sorted(source.skills, key=skill_score, reverse=True)
