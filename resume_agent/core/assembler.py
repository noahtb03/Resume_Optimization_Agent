...
from ..models.output import (
    CoverageReport, ResumeMeta, SkillGroupOut, SummaryBlock, TailoredBullet,
    TailoredExperience, TailoredProject, TailoredResume, ValidationStatus,
)
...
def assemble(
    ...
    allowed_skill_ids = set(selection.skills_section_ids)

    # job-adaptive skill groups from the model, validated: every id must be an
    # APPROVED, selected skill; unknown ids are dropped; empty groups removed.
    skill_groups: list[SkillGroupOut] = []
    used_ids: set[str] = set()
    for g in output.skill_groups:
        members = [skills_by_id[sid] for sid in g.skill_ids
                   if sid in allowed_skill_ids and sid in skills_by_id and sid not in used_ids]
        if members:
            skill_groups.append(SkillGroupOut(label=g.label.strip() or "Skills", skills=members))
            used_ids.update(s.id for s in members)
    # any selected skills the model didn't place -> appended to a generic group
    leftover = [s for s in skills if s.id not in used_ids]
    if leftover and skill_groups:
        skill_groups.append(SkillGroupOut(label="Additional", skills=leftover))
    # FALLBACK: no usable groups -> group by parse-time category
    if not skill_groups:
        by_cat: dict[str, list] = {}
        for s in skills:
            by_cat.setdefault(s.category or "Skills", []).append(s)
        skill_groups = [SkillGroupOut(label=cat, skills=members) for cat, members in by_cat.items()]
